...
export async function streamArticle(
  ...
  // Calculate maxTokens (slightly reduced for faster streaming)
  const baseMaxTokens = calculateMaxTokens(source, platformSettings);
  const maxTokens = Math.min(Math.ceil(baseMaxTokens * 0.9), 8192);

  let platformContent: PlatformContent;
  let jsonResponse: string | undefined;

  try {
    console.log(`Streaming ${source} content generation from AI`);
    // Stream content generation from AI
